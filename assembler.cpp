...
{ // struct to handle litterals
    ...
    LitInfo(string litName)
    {
        isString = true;
        if (litName == "*")
            value = getLocctrS(); // if =*, set value to locctr
        else
            value = getObjectCodeOfString(litName); // otherwise get object code
...
unordered_map<string, int> length, startingAddresses; // storing lengths and starting addresses of sections
...
// int startingAddress;
int locctr; // Initializing locctr
int base;
string line;
...
void setBit(string &hexString, int pos)
...
{ // trim whitespaces from begin and end
...
void makeStringEvenLength(string &s)
{ // pad string to make it even length
    if (s.size() % 2)
        s = "0" + s;
}
...
{                           // struct to handle Modification Records
    ...
    void addRecord(string &address, int length, char sign, string &name)
    ...
    void writeToStream(ofstream &fout)
...
{ // Find value of operand
    ...
    {
        operand = symtabs[subRoutineName][operand]; // If found in symtab, return value
    }
    else if (operand[0] == '=')
    {
...
{                                // Helper function to get object codes of
    ...
    int pc = locctr + 3;
    if (address - pc >= -2048 && address - pc < 2048)
    { // Checks if PC relative addressing works
        operand = getHex2Comp(address - pc);
        setBit(operand, 13);
    }
    else if (address - base >= -2048 && address - base < 2048)
...
void handleAddressType4(string &operand, bool isImmediate, string address)
{ // Helper function to get object codes of instruction of type 4
    if (!isImmediate)
    { 
...
struct Instruction
{ // Struct to hold Input assembly instructions
    ...
    int getOperandInt()
    { // Converts operand to int
        return stoi(operand);
    }
    int getLengthByteArg()
...
{ // Struct to hold Text Records
    ...
    void addObjectCode(ofstream &fout, string objectCode, string address)
    ...
    void writeRecordOnStream(ofstream &fout)
...
{ // get address opcode from instruction operand
    ...
    { // else but operand as 0 and errorFlag=true
        operand = "0000";
        setErrorFlag();
        return;
    }
...
void writeStringToStream(ofstream &fout, string &objectCode, InstructionInter &inst)
{
    ...
    int j = 0;
    for (int i = 0; i < sz - 6; i += 6)
    {                                                         // Breaks BYTE string to 3 byte blocks
...
//function to check if the expression is valid or not according to directives in book
int check_valid_expression1(string expression)
{
    string temp;
    ...
    for (auto s : v)
    {
        if (symtabs[subRoutineName].count(s))
        {
            n1++;
        }
        else
    ...
    //if all terms are relative
    if (n1 == y)
    {
        return 1;
    }
    //if all terms are absolute
    else if (n2 == y)
    {
        return 2;
    }
...
{ // Find value of expression
    long val = 0;
    if (check_valid_expression1(expression) == 1)
    {
        string temp;
        ...
        { // Iterating through the expression
            ...
            {
                long num;
                if (symtabs[subRoutineName].count(temp))
                    num = hexToInt(symtabs[subRoutineName][temp]);
                if (lastSym == '-')
    ...
    else if (check_valid_expression1(expression) == 2)
    {
        string temp;
        char lastSym = '+'; // Doing initializations
        expression += '+';
...
void writeLitToStream(ofstream &fout, LitInfo &litInfo, string name)
...
void writeLitsToStream(ofstream &fout)
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2)
    { // Taking input filename as operand
        cout << "\nInvalid number of arguments\nSee README.md for Steps to run\nABORTING!!!\n\n";
        return -1;
    }
    ...
    while (!finput.eof())
    {
        ...
        if (line.size() == 0)
            break;
        if (line[0] == '.')
        { // It is a comment, so just write in intermediate file
            fileout << line << '\n';
            continue;
        }
        ...
        else
        {
            ...
            if (inst.opcode != "END")
            {
                ...
                else if (inst.opcode == "EQU")
                {
                    ...
                    {
                        if (check_valid_expression1(inst.operand) != -1)
